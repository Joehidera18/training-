...
def connect(path):
    ...
    # Better crash/restart resilience for the checkpoint database.
...
def init_db(path):
    ...
    con.executescript("""
      ...
      CREATE TABLE IF NOT EXISTS counterfactual_memory(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        run_id INTEGER, symbol TEXT NOT NULL, interval TEXT NOT NULL, ts INTEGER NOT NULL,
        direction TEXT NOT NULL, rejection_reason TEXT, features_json TEXT NOT NULL, outcome_json TEXT NOT NULL,
        UNIQUE(symbol,interval,ts,direction,rejection_reason)
      );
      CREATE INDEX IF NOT EXISTS idx_counterfactual_scope ON counterfactual_memory(symbol,interval,ts);
    """)
