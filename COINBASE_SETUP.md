...
Click Use Coinbase fee in research. The Settings form refreshes to the synced taker rate. A change invalidates profiles whose tested fee differs. Rerun historical research with the actual rate and collect forward paper results. Additional tax or cost-plus fee schemes are unsupported and block live entries.

Start the paper/market runner to maintain completed candles and features. Then start the Coinbase preview runner. Both services have separate controls. Signals require a current passing historical profile and matching current features, even if paper experiment mode is enabled.
...
| Reentry | 15 minutes normally; 6 hours after three consecutive net losing closes under V10 rules |
