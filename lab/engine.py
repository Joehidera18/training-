...
ENGINE_VERSION="market-structure-v5.2"
    ...
    for i in range(max(start,240),min(end,len(rows)-1)):
        ...
        if pos is None and f and equity>1:
            ...
            if sc is None:
                ...
                signal_price=rows[i]["close"]
                entry_mode=params.get("entry_mode","MARKET_NEXT_OPEN")
                if entry_mode=="RETRACE_LIMIT":
                    if direction=="LONG":
                        raw=f.get("bull_fvg_mid")
                        floor=signal_price-.22*f["_atr"]
                        desired=max(floor,raw) if raw is not None else signal_price-.15*f["_atr"]
                        desired=min(desired,signal_price)
                    else:
                        raw=f.get("bear_fvg_mid")
                        ceiling=signal_price+.22*f["_atr"]
                        desired=min(ceiling,raw) if raw is not None else signal_price+.15*f["_atr"]
                        desired=max(desired,signal_price)
                    if not (nxt["low"]<=desired<=nxt["high"]):
                        reject("retrace_limit_not_filled");continue
                    provisional=desired
                    entry_slip=min(dynamic_slippage(nxt,1.0,base_slip),base_slip+.0005)
                else:
                    provisional=nxt["open"]
                    gap_atr=abs(provisional-signal_price)/max(f["_atr"],1e-9)
                    if gap_atr>params.get("max_gap_atr",99):
                        reject("entry_gap_too_large");continue
                ...
                pos={"entry_ts":nxt["ts"],"entry":entry,"signal_entry":signal_price,
                     ...
                     "direction":direction,"strategy_family":params["family"],"entry_mode":entry_mode,
