            ...
            for stop,rr2,vol,confirm in configs:
                for threshold in (64,70):
                    out.append({"family":fam,"direction":direction,"threshold":threshold,
                      ...
                      "max_cost_r":.50,"time_stop_hours":24,"confirmation":confirm})
            ...
            for stop,rr2,confirm in ((1.15,1.8,0),(1.45,2.2,1),(1.75,2.6,2)):
                for threshold in (62,68):
                    out.append({"family":fam,"direction":direction,"threshold":threshold,
                      ...
                      "max_cost_r":.46,"time_stop_hours":20,"confirmation":confirm})
            ...
            for stop,rr2,confirm in ((1.0,1.7,0),(1.25,2.1,1),(1.55,2.5,2)):
                for threshold in (60,66):
                    out.append({"family":fam,"direction":direction,"threshold":threshold,
                      ...
                      "max_cost_r":.42,"time_stop_hours":16,"confirmation":confirm})
        else:  # mean reversion
            for stop,rr2 in ((1.0,1.35),(1.25,1.65),(1.5,1.9)):
                for threshold in (60,66):
                    out.append({"family":fam,"direction":direction,"threshold":threshold,
                      ...
                      "max_cost_r":.38,"time_stop_hours":12,"confirmation":1})
    uniq={}
...
def evaluate_signal(f,p,edge_prob=None,edge_lower=None,edge_samples=0):
    ...
    if fam=="trend_breakout":
        ...
        if f.get("daily_bias_score",0)<-.15:return None,"daily_bias_conflict"
        ...
        score += 6 if f["displacement_atr"]>.65 else 0
        score += 5 if f["vwap_distance_atr"]>0 else -4
        score += 4 if f["equal_highs"]>=2 else 0
        score += 9 if f.get("bos_up") else 0
        score += 6 if f.get("bull_fvg_active") else 0
        score += 5 if f.get("discount") else 0
        if not f["breakout"]:return None,"no_range_breakout"

    elif fam=="trend_pullback":
        if f["regime"]!="BULL":return None,"wrong_regime"
        if f.get("daily_bias_score",0)<0:return None,"daily_bias_conflict"
        ...
        score += 8 if f.get("discount") else 0
        score += 8 if f.get("bos_up") or f.get("choch_up") else 0
        ...
        score += 10 if f["sweep_depth_atr"]>=.10 else 0
        score += 6 if f["equal_lows"]>=2 else 0
        score += 5 if f["displacement_atr"]>.45 else 0
        score += 12 if f.get("liquidity_sweep_low") else 0
        score += 8 if f.get("choch_up") or f.get("bos_up") else 0
        score += 6 if f.get("discount") else 0
        score += 5 if f.get("bull_fvg_active") else 0
        if not (f["sweep_low"] or f.get("liquidity_sweep_low")):return None,"no_liquidity_sweep"
        ...
        score += 7 if f["vwap_distance_atr"]<-1.0 else 0
        if f["adx"]>24:return None,"trend_too_strong_for_mean_reversion"
        ...
        score += 6 if f["displacement_atr"]>.65 else 0
        score += 5 if f["vwap_distance_atr"]<0 else -4
        score += 4 if f["equal_lows"]>=2 else 0
        score += 9 if f.get("bos_down") else 0
        score += 6 if f.get("bear_fvg_active") else 0
        score += 5 if f.get("premium") else 0
        ...
        score += 8 if f["signed_volume_pressure"]<=.03 else -5
        score += 6 if adx_ok else 0
        score += 8 if f.get("premium") else 0
        score += 8 if f.get("bos_down") or f.get("choch_down") else 0
        score += 5 if f.get("bear_fvg_active") else 0
        if not f["_pullback_short"]:return None,"no_bear_rally"

    if f.get("micro_available"):
        if direction=="LONG":
            score += 7 if f.get("aggressor_imbalance",0)>.12 else (-5 if f.get("aggressor_imbalance",0)<-.18 else 0)
            score += 5 if f.get("book_imbalance",0)>.10 else (-4 if f.get("book_imbalance",0)<-.15 else 0)
            score += 4 if f.get("liquidation_imbalance",0)>.15 else 0
        else:
            score += 7 if f.get("aggressor_imbalance",0)<-.12 else (-5 if f.get("aggressor_imbalance",0)>.18 else 0)
            score += 5 if f.get("book_imbalance",0)<-.10 else (-4 if f.get("book_imbalance",0)>.15 else 0)
            score += 4 if f.get("liquidation_imbalance",0)<-.15 else 0
        if f.get("spread_bps",0)>12: score-=8
        if abs(f.get("funding_rate",0))>.0015: score-=3
...
def robustness(m):
    # No-trade is better than selecting a validated loser.
    if m["trades"]<MIN_TRAIN_TRADES:return -1e9
    pf=float(m["profit_factor"] or 0);exp=float(m["expectancy_r"] or -9);dd=float(m["max_drawdown_pct"] or 100)
    if exp<=0.02 or pf<1.03 or dd>28:return -1e9
    return exp*65+min(pf,3)*18-dd*.85+min(m["trades"],100)*.05
...
def finalize_result(db_path,config,state):
    ...
    result={"symbol":config["symbol"],"interval":config["interval"],"rows":state["rows"],
            ...
            "architecture":{"resumable":True,"candidate_count":len(state["candidates"]),
                            ...
                            "selection":"nested validation; no-edge/no-trade gate"},
