...
from .memory import load_memory_trades,store_resolved_trades,memory_summary,evaluate_challenger
...
def evaluate_signal(f,p,edge_prob=None,edge_lower=None,edge_samples=0):
    """
    Return (score, rejection_reason). A non-None score means the candle qualifies.
    The reason is used to build a signal funnel so zero-trade runs are explainable.
    """
    if not f:
    ...
    if not f["breakout"] and 0<=f["distance_to_resistance_atr"]<.55:
        score-=14
    elif f["distance_to_resistance_atr"]>=1.25:
        score+=6

    # Do not hard-veto an otherwise good chart purely from a noisy small-sample
    # probability. Penalize weak evidence and let the total setup score decide.
    if edge_prob is not None:
        score+=(edge_prob-.5)*24
        if edge_samples>=40 and edge_lower is not None:
            if edge_lower < p["min_edge_probability"]-.10:
                score-=12
            elif edge_lower < p["min_edge_probability"]-.05:
                score-=6

    if score < p["threshold"]:
        # Identify the strongest explanatory bottleneck, rather than just "threshold".
        if f["regime"]=="BEAR": return None,"bear_regime"
        if not f["_trend"] and f["mtf_alignment"]<=0: return None,"trend_alignment"
        if not (p["rsi_min"]<=f["rsi"]<=p["rsi_max"]): return None,"rsi"
        if f["volume_z"]<p["volume_z_min"]: return None,"volume"
        if not (f["sweep_low"] or f["breakout"] or f["compression"] or f["rejection_strength"]>.25):
            return None,"no_setup_trigger"
        if not f["breakout"] and 0<=f["distance_to_resistance_atr"]<.55:
            return None,"near_resistance"
        if edge_prob is not None and edge_samples>=40 and edge_prob < p["min_edge_probability"]:
...
def _signal_score(f,p,edge_prob=None,edge_lower=None,edge_samples=0):
    score,_=evaluate_signal(f,p,edge_prob,edge_lower,edge_samples)
    return score
...
def simulate(rows,features,start,end,balance,risk,fee_rate,base_slip,params,edge_model=None,keep_trades=True):
    ...
    funnel={
      "candles_checked":0,"features_available":0,"qualified_setups":0,"entry_attempts":0,
      "entries_opened":0,"rejections":{}
    }
    ...
    for i in range(max(start,240),min(end,len(rows)-1)):
        ...
        if pos is None and f:
            ...
            if sc is None:
                ...
                if dist<=0:
                    funnel["rejections"]["invalid_stop_distance"]=funnel["rejections"].get("invalid_stop_distance",0)+1
                else:
                    qty=min(cash*risk/dist,(cash*.98)/(provisional*(1+fee_rate+base_slip)))
                    if qty<=0:
                        funnel["rejections"]["position_size_zero"]=funnel["rejections"].get("position_size_zero",0)+1
                    else:
                        sl=dynamic_slippage(nxt,qty,base_slip);entry=provisional*(1+sl);dist=entry-stop
                        if dist<=0:
                            funnel["rejections"]["fill_invalidated_stop"]=funnel["rejections"].get("fill_invalidated_stop",0)+1
                        else:
                            ...
                            if qty<=0 or cost>cash:
                                funnel["rejections"]["insufficient_cash"]=funnel["rejections"].get("insufficient_cash",0)+1
                            else:
                                cash-=cost;funnel["entries_opened"]+=1
    ...
    # Sort bottlenecks so the UI can immediately show what killed setups.
...
def finalize_result(db_path,config,state):
    ...
    # Permanent learning happens only AFTER the run is fully resolved.
    newly_learned=store_resolved_trades(db_path,rid,config["symbol"],config["interval"],all_trades)
    decision,champion=evaluate_challenger(
        db_path,rid,config["symbol"],config["interval"],best,pooled,mc
    )
    result["persistent_learning"]={
        "new_unique_trades_learned":newly_learned,
        "memory":memory_summary(db_path,config["symbol"],config["interval"]),
        "challenger_decision":decision,
        "champion":champion
    }
    # Re-save the run JSON with learning decision included.
    con=None
    try:
        from .db import connect
        con=connect(db_path)
        con.execute("UPDATE runs SET result_json=? WHERE id=?",
                    (json.dumps(result,separators=(",",":"),default=str),rid))
        con.commit()
    finally:
        if con: con.close()
    return result
