...
def simulate(rows, features, start, end, balance, risk, fee_rate, base_slip,
             params, edge_model=None, keep_trades=True):
    ...
    def close(raw, candle, reason):
        ...
        next_entry_ts = candle["ts"] + bar_ms + cooldown_minutes([t["pnl"] for t in trades],params) * 60000
        position = None
    ...
    for i in range(max(240, int(start)), end - 1):
        ...
        if not held_at_open and f and cash > 1 and candle["ts"] >= next_entry_ts:
            ...
            score, reason = evaluate_signal(f, params, detail.get("probability"),
            ...
            if score is None:
                ...
                mode = params.get("entry_mode", "MARKET_NEXT_OPEN")
                raw = candle["open"]
                if mode == "RETRACE_LIMIT":
                    ...
                    if not retrace:
                        ...
                        raw = None
                if raw is not None and abs(raw - signal["close"]) / max(atr, 1e-12) > params.get("max_gap_atr", .6):
                    reject("entry_gap_too_large")
                    raw = None
                if raw is not None:
                    entry = raw * (1 + sign * base_slip)
                    distance = max(atr * params["stop_atr"], raw * .0015)
                    ...
                    if stop <= 0 or target <= 0 or not math.isfinite(unit_risk) or unit_risk <= 0:
                    ...
                    elif cost_r > params.get("max_cost_r", .8):
                        reject("trading_cost_too_high")
                    elif quality["net_rr"] < params.get("min_net_rr",0):
                        reject("net_reward_too_small")
                    else:
                        fraction = min(1.0, params.get("max_notional_fraction", .30))
                        ...
                        if qty * entry < 1:
                            ...
                            position = {"entry_ts": candle["ts"], "entry": entry, "signal_entry": signal["close"],
                                ...
                                "strategy_family": params["family"], "entry_mode": mode,
        ...
        if position:
            ...
            if (candle["open"] <= p["stop"] if sign == 1 else candle["open"] >= p["stop"]):
                ...
                if stop_hit:
                    ...
                    if candle["ts"] - p["entry_ts"] >= params.get("time_stop_hours", 24) * 3600000:
