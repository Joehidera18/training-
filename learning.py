...
FEATURES=("rsi","volume_z","sweep_low","breakout","compression","trend_strength","atr_pct",
          ...
          "vwap_distance_atr","sweep_depth_atr","displacement_atr","signed_volume_pressure","adx","weekend","hour_block",
          "book_imbalance","aggressor_imbalance","liquidation_imbalance","funding_rate","basis_pct","spread_bps",
          "daily_bias_score","bos_up","bos_down","choch_up","choch_down","equilibrium_position",
          "liquidity_sweep_low","liquidity_sweep_high","bull_fvg_active","bear_fvg_active",
...
def _bin(name,v):
    ...
    if name in ("distance_to_resistance_atr","vwap_distance_atr"):return round(max(-6,min(12,x))*2)/2
    if name in ("sweep_depth_atr","displacement_atr"):return round(max(0,min(5,x))*4)/4
    if name=="signed_volume_pressure":return round(max(-1,min(1,x))*10)/10
    if name=="adx":return int(max(0,min(60,x))//5)*5
    if name in ("weekend","hour_block"):return int(x)
    if name in ("book_imbalance","aggressor_imbalance","liquidation_imbalance"):
        return round(max(-1,min(1,x))*10)/10
    if name=="funding_rate":return round(max(-.01,min(.01,x))*10000)/10000
    if name=="basis_pct":return round(max(-10,min(10,x))*2)/2
    if name=="spread_bps":return int(max(0,min(100,x))//2)*2
    if name=="daily_bias_score":return round(max(-1,min(1,x))*10)/10
    if name=="equilibrium_position":return round(max(0,min(1,x))*10)/10
    if name in ("bos_up","bos_down","choch_up","choch_down","liquidity_sweep_low",
                "liquidity_sweep_high","bull_fvg_active","bear_fvg_active",
                "bullish_reversal_sequence","bearish_reversal_sequence",
                "bullish_continuation_sequence","bearish_continuation_sequence"):return bool(x)
    return round(x,2)
