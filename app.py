...
def not_found(err):
    if request.path.startswith("/api/"):
        return jsonify({"error":"API route not found","code":"API_NOT_FOUND","path":request.path}),404
    return err

@app.errorhandler(500)
def internal_error(err):
    if request.path.startswith("/api/"):
        return jsonify({"error":"Internal server error","code":"API_INTERNAL_ERROR"}),500
    return err
...
def health():return {
    "ok":True,"app":"CryptO Research Lab","mode":"resumable","api_version":"5.5",
    "checkpoint_db":str(DB_PATH),
    "persistent_storage_configured":bool(os.getenv("RESEARCH_DB_PATH") and os.getenv("RESEARCH_DATA_DIR"))
}
@app.route("/api/runs")
...
def step(jid):
    ...
    if not j:
        replacement=active_job(DB_PATH)
        return jsonify({
            "error":"job checkpoint not found",
            "code":"JOB_CHECKPOINT_LOST",
            "recoverable":True,
            "replacement_job_id":replacement["id"] if replacement else None
        }),410
        ...
        if j["stage"]=="prepare":
            ...
            if not eligible:
                ...
                s["fold_results"].append({
                    ...
                    "validation":most["metrics"],"train":most["metrics"],
                    "test":{"trades":0,"signal_funnel":{"rejections":{"no_positive_validated_edge":1}}},
                    "message":"No strategy family or sequence produced positive validated expectancy; outer test skipped."
                })
...
def concepts_api():
    return jsonify({
      "structure":["HH/HL/LH/LL","BOS","CHOCH/MSS","swing highs/lows"],
      "liquidity":["equal highs/lows","previous-day/week highs/lows","sweeps","reclaims"],
      "imbalances":["FVG","IFVG-style invalidation","displacement"],
      "location":["equilibrium","premium","discount"],
      "context":["daily bias score","regime","session/hour block","weekend"],
      "cross_market":["SMT/relative-strength helper module"],
      "learning":["persistent mistakes","counterfactual rejected setups","sequence expectancy","probability calibration"]
    })
